...
private:
	...
	double kernel_rq(int i, int j) const
	{
 ...
 gamma(param.gamma), coef0(param.coef0)
{
	switch(kernel_type)
	{
		...
		case PRECOMPUTED:
			...
			break;
		case RQ:
			kernel_function = &Kernel::kernel_rq;
			break;
	}

	clone(x,x_,l);

	if(kernel_type == RBF || kernel_type == RQ)
	{
...
{
	"linear","polynomial","rbf","sigmoid","precomputed","rq",NULL
};
...
int svm_save_model(const char *model_file_name, const svm_model *model)
{
	...
	if(param.kernel_type == POLY || param.kernel_type == SIGMOID|| param.kernel_type == RQ)
