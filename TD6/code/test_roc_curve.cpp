...
int main(int argc, char* argv[]) {
    ...
    for (int j = 0; j < class_dataset.getNbrSamples(); j++) {
        ANNpoint x = annAllocPt(class_dataset.getDim() - 1);
        int z = 0;
        for (int b = 0; b < class_dataset.getDim(); b++) {
            if (b == col_class)continue;
            x[z] = class_dataset.getInstance(j)[b];
            z++;
        }
        for (int i = 0; i < num_points; i++) {
            confusion_matrices[i].AddPrediction(class_dataset.getInstance(j)[col_class], knn_class.Estimate(x, thresholds[i]));}
    }
