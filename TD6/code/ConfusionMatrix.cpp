...
double ConfusionMatrix::f_score() const {
    return ((1.0) * 2 * precision() * detection_rate()) / (precision() + detection_rate());
}

double ConfusionMatrix::precision() const {
    return ((1.0) * GetTP()) / (GetTP() + GetFP());
}

double ConfusionMatrix::error_rate() const {
    return (1.0)*(GetFP() + GetFN())/(GetFP()+GetFN()+GetTP()+GetTN());
}

double ConfusionMatrix::detection_rate() const {
    return ((1.0) * GetTP()) / (GetTP() + GetFN());
}

double ConfusionMatrix::false_alarm_rate() const {
    return ((1.0) * GetFP()) / (GetFP() + GetTN());
}
